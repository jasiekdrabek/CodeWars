...
function counterOfWordsBefore(map, key, positonLeft) {
    let res = 0;
    let sum = 1;
    let factorialPositonLeft = factorialize(positonLeft);
    if (positonLeft == 0) { return 0 };
    for (const k of map.keys()) {
        sum *= factorialize(map.get(k));
    }
    for (const k of map.keys()) {
        if (k == key) {
        ...
        res += map.get(k) * factorialPositonLeft / sum
    }
}
function listPosition(word) {
    ...
    for (let i = 0; i < word.length; i++) {
        res += counterOfWordsBefore(map, word[i], word.length - (i + 1));
